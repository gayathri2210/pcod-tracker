...
# --- ML Artifact Loading (LOGISTIC MODEL) ---
ml_model = None
...
def load_ml_artifacts():
    ...
    try:
        if not os.path.exists(MODEL_PATH): print(f"FATAL ERROR: Model file NOT FOUND: {MODEL_PATH}"); return False
        if not os.path.exists(TRAINING_COLS_PATH): print(f"FATAL ERROR: Training columns NOT FOUND: {TRAINING_COLS_PATH}"); return False
        ...
        return True
    except Exception as e: print(f"FATAL ERROR loading LOGISTIC artifacts: {e}"); traceback.print_exc(); return False
...
@login_required
def profile():
    return render_template('profile.html', title=f"{current_user.username}'s Profile", user=current_user)
...
def predict_pcos_via_form():
    ...
    if not ml_model or not ml_training_columns:
        print("[PREDICT STEP 0] Logistic Model or columns not loaded."); flash('Model not available.', 'danger')
    ...
    for model_col_name in ml_training_columns:
        ...
        # *** UPDATED (Y/N) HANDLING BASED ON INSPECTION OUTPUT ***
        elif model_col_name == 'Fast food (Y/N)':
            if html_form_key in submitted_form_data:
                processed_value = "1.0" # Model learned "1.0" for Yes-like category
            else:
                processed_value = "0.0" # Model learned "0.0" for No-like category
        ...
        elif model_col_name.endswith('(Y/N)'): # For all other Y/N symptoms
            if html_form_key in submitted_form_data:
                processed_value = "1" # Model learned "1" for Yes-like category
            else:
                processed_value = "0" # Model learned "0" for No-like category
        ...
        # *** END OF UPDATED (Y/N) HANDLING ***
            
        else: # Assume other features are numeric
            raw_form_val = submitted_form_data.get(html_form_key)
    ...
    print("[PREDICT STEP 4] Starting prediction...")
    try:
    ...
    PCOS_PROBABILITY_THRESHOLD = 0.30 # Adjust as needed
